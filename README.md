...
- [dbchat-3.0.4.jar) - Basic version (PostgreSQL, SQLite, H2, HSQLDB, CSV)   `<-- Start here`
- [dbchat-3.0.4.jar) - Standard version (add MySQL, MariaDB, ClickHouse)
- [dbchat-3.0.4.jar) - Enterprise version (add Oracle, SQL Server, DB2)
- [dbchat-3.0.4.jar) - Cloud Analytics version (add Redshift, Snowflake, BigQuery)
- [dbchat-3.0.4.jar) - All databases included (400MB+)

(Optional) For simplicity rename the downloaded file to `dbchat-3.0.4.jar` so you can follow the rest of the commands here without needing to adjust them to your version.
```agsl
mv dbchat-3.0.4.jar
OR
ren dbchat-3.0.4.jar
```
...
```json
{
  "mcpServers": {
    "database": {
      ...
      "args": [
        "-jar", 
        "/absolute/path/to/dbchat-3.0.4.jar",
...
```json
{
  "mcpServers": {
    "database": {
      "command": "java",
      "args": ["-jar", "/absolute/path/to/dbchat-3.0.4.jar"],
      "env": {
        "DB_URL": "jdbc:mysql://localhost:3306/your_database",
        "DB_USER": "your_username", 
        "DB_PASSWORD": "your_password",
...
```json
{
  "mcpServers": {
    "database": {
      ...
      "args": [
        "-jar", 
        "C:/Users/YourName/Downloads/dbchat-3.0.4.jar",
...
```json
{
  "mcpServers": {
    "production-db": {
      "command": "java",
      "args": ["-jar", "/path/to/dbchat-3.0.4.jar"],
      "env": {
...
java -jar dbchat-3.0.4.jar --http_mode=true --http_port=8080

# Bind to all interfaces (allows external access)
java -jar dbchat-3.0.4.jar --http_mode=true --bind_address=0.0.0.0 --http_port=8080

# Bind to specific interface
java -jar dbchat-3.0.4.jar --http_mode=true --bind_address=192.168.1.100 --http_port=8080
```
...
**Usage:**
```bash
java -jar dbchat-3.0.4.jar --config_file=dbchat.conf
```
...
java -jar dbchat-3.0.4.jar --config_file=config.conf --http_port=7070
...
java -jar dbchat-3.0.4.jar --config_file=prod.conf --http_port=9090
```
...
export DB_PASSWORD="secure_password"
java -jar dbchat-3.0.4.jar --config_file=app.conf

# Good: Use config file with restricted permissions
chmod 600 secure.conf
java -jar dbchat-3.0.4.jar --config_file=secure.conf

# Avoid: Passwords in command line (visible in process list)
java -jar dbchat-3.0.4.jar --db_password="visible_password"
```
...
```bash
java -Dlogging.level.root=DEBUG -jar dbchat-3.0.4.jar --config_file=myconfig.conf
```
...
java -jar dbchat-3.0.4.jar --help
