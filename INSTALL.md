...
**Resulting JAR:** `target/dbchat-3.0.4.jar` (~15MB)
...
**Resulting JAR:** `target/dbchat-3.0.4.jar` (~25MB)
...
**Resulting JAR:** `target/dbchat-3.0.4.jar` (~60MB)
...
**Resulting JAR:** `target/dbchat-3.0.4.jar` (~80MB)
...
**Resulting JAR:** `target/dbchat-3.0.4.jar` (~120MB)
...
**Resulting JAR:** `target/dbchat-3.0.4.jar` (~400MB)
...
ls -la target/dbchat-3.0.4.jar

# Check included drivers
jar tf target/dbchat-3.0.4.jar | grep -E "\.(jar|class)" | grep -E "(mysql|postgres|oracle)"

# Quick test to see if it starts
java -jar target/dbchat-3.0.4.jar
...
java -Dlogging.level.root=DEBUG -jar target/dbchat-3.0.4.jar

# Test with manual input
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26","capabilities":{},"clientInfo":{"name":"test","version":"1.0"}}}' | java -jar target/dbchat-3.0.4.jar
```

#### HTTP Development Mode
```bash
# Basic HTTP mode
java -jar target/dbchat-3.0.4.jar --http_mode=true

# Custom port with debug logging
java -Dlogging.level.root=DEBUG -jar target/dbchat-3.0.4.jar --http_mode=true --http_port=9090
...
java -jar target/dbchat-3.0.4.jar --config_file=production.conf

# Override specific settings
java -jar target/dbchat-3.0.4.jar --config_file=production.conf --http_port=9090

# Multiple config files for different environments
java -jar target/dbchat-3.0.4.jar --config_file=base.conf --config_file=env-specific.conf
```
...
echo '{"jsonrpc":"2.0","method":"notifications/initialized"}' | java -jar target/dbchat-3.0.4.jar

# List tools
echo '{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}' | java -jar target/dbchat-3.0.4.jar

# Execute query
echo '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"query","arguments":{"sql":"SELECT 1 as test","maxRows":1}}}' | java -jar target/dbchat-3.0.4.jar

# List resources
echo '{"jsonrpc":"2.0","id":4,"method":"resources/list","params":{}}' | java -jar target/dbchat-3.0.4.jar
```
...
mvn clean package -q
test -f target/dbchat-3.0.4.jar || (echo "Build failed" && exit 1)
...
test -f target/dbchat-3.0.4.jar || (echo "Standard build failed" && exit 1)
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26","capabilities":{},"clientInfo":{"name":"test","version":"1.0"}}}' | \
    timeout 10s java -jar target/dbchat-3.0.4.jar > /dev/null || (echo "Functionality test failed" && exit 1)
...
COPY --from=build /app/target/dbchat-3.0.4.jar
...
ExecStart=/usr/bin/java -jar /opt/dbchat/dbchat-3.0.4.jar --config_file=/etc/dbchat/production.conf
Restart=always
RestartSec=10
...
# Copy files
sudo cp target/dbchat-3.0.4.jar /opt/dbchat/
...
jar tf target/dbchat-3.0.4.jar | grep -E "(mysql|postgres|oracle)"

# Test driver loading
java -cp target/dbchat-3.0.4.jar -e "Class.forName('com.mysql.cj.jdbc.Driver')"
...
java -cp target/dbchat-3.0.4.jar -e "
import java.sql.*;
...
java -jar target/dbchat-3.0.4.jar --db_url="$DB_URL" --help
```

#### Memory Issues
```bash
# Monitor memory usage
java -Xmx512m -XX:+PrintGCDetails -jar target/dbchat-3.0.4.jar

# Profile memory allocation
java -XX:+UseG1GC -XX:+PrintGCApplicationStoppedTime -jar target/dbchat-3.0.4.jar

# Enable heap dump on OOM
java -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp -jar target/dbchat-3.0.4.jar
```
